...
| 客户端流式 | ✅ 打 Go 服务端实测 |
| 拦截器（一元 + 流式同一条链） | ✅ |
| deadline 传递（grpc-timeout） | ✅ 服务端主动掐并回 DEADLINE_EXCEEDED |
| 多后端轮询 + 断线重连 + 重试 | ✅ 杀掉一个后端请求不掉 |
| 客户端拦截器 | ✅ 跟服务端一条路子 |
| 元数据（读/发/响应 trailer） | ✅ 鉴权实测 |
| deadline 传递 | ✅ 服务端主动掐 |
| 消息大小上限（默认 4MB） | ✅ 收发两侧 |
| 出站背压 | ✅ 有界队列 |
| 断连探测 / 优雅停机 | ✅ SIGTERM 后在途请求做完再退 |
| gzip + deflate | ✅ |
| 反射四种请求（含扩展） | ✅ |
| 健康退避的多后端均衡 | ✅ 死掉的后端撞一次就进退避 |
| 服务发现（DNS/xDS）| ❌ 只支持一组固定地址 |
| 加权均衡 / 客户端流的自动重试 | ❌ |
...
形状跟 qi-web 的中间件一样，**后加的先跑**（洋葱）：

```qi
函数 记一笔(下一步: 函数(整数): 整数, 调用: 整数) : 整数 {
    变量 开始: 整数 = 时间.现在毫秒();
    变量 结局: 整数 = 下一步(调用);          // 不调它 = 拦住
    IO.打印行(方法名(调用) + " 用了 " + 整数转字符串(时间.现在毫秒() - 开始) + " 毫秒");
    返回 结局;
}
加拦截器(记一笔);
```

一元和流式走**同一条链** —— 鉴权、日志、限流不该因为方法是不是流式而写两套。

## deadline 与多后端

客户端把超时作为 `grpc-timeout` 发过去；服务端**自己**掐并回
`DEADLINE_EXCEEDED`，不是干等客户端那侧超时（那样服务端日志上什么都看不到）。
handler 里可以问 `剩余预算(调用)`：只剩两百毫秒就别开工了，干完也没人要。

客户端一个句柄可以挂多个后端，逗号分隔：

```qi
变量 连: 整数 = 连接后端("10.0.0.1:50051,10.0.0.2:50051");
```

轮询挑一个；子连接坏了下次用之前重连；**只对 UNAVAILABLE 重试**（退避 100/200ms）——
那意味着请求根本没被对面处理。别的错都可能是「已经执行了一半」，重试就是
重复扣款那类事故。
...
这条已经接进 qi 的 `make ci` 和 GitHub CI（macOS + Ubuntu 都真跑）——
gRPC 的故障方式是「挂着不动」和「静默丢消息」，不是干脆的崩溃，
别的测试全绿也照样发现不了。
